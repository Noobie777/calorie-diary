...
def delete_log(id: int, db: Session = Depends(get_db)):
    log = crud.delete_log(db, id)
    if not log:
        raise HTTPException(status_code=404, detail="Log not found")
    return {"message": "Log deleted"}

@router.put("/logs/{id}",response_model=FoodLogRead)
def update_log(id:int, updated_log: FoodLogCreate, db: Session = Depends(get_db)):
    log = crud.update_log(db, id, updated_log)
    if not log:
        raise HTTPException(status_code=404, detail="Log not found")
    return log

@router.patch("/logs/{id}",response_model=FoodLogRead)
def patch_log(id: int, updated_log: FoodLogUpdate, db: Session = Depends(get_db)):
    log = crud.patch_log(db, id, updated_log)
    if not log:
        raise HTTPException(status_code=404, detail="Log not found")
    return log